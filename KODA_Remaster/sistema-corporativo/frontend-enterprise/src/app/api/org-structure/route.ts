...
async function proxy(request: Request, method: "GET" | "PUT") {
  ...
  for (const base of urls) {
    try {
      const response = await fetch(`${base}/org-structure`, {
        method,
...
export async function GET(request: Request) {
  try {
    ...
    console.error("Org structure GET proxy error:", error);
    return NextResponse.json({ detail: "Error en el proxy de estructura organizativa" }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    console.error("Org structure PUT proxy error:", error);
    return NextResponse.json({ detail: "Error en el proxy de estructura organizativa" }, { status: 500 });
  }
}
