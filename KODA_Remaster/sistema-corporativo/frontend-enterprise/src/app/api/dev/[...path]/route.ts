...
async function proxyToBackend(
  ...
  for (const base of urls) {
    try {
      ...
      const response = await fetch(`${base}/dev/${subPath}${search}`, {
        method,
...
function errorResponse(method: string, error: unknown) {
  console.error(`Dev API ${method} proxy error:`, error);
  return NextResponse.json({ detail: "Error en el proxy de desarrollo" }, { status: 500 });
}
