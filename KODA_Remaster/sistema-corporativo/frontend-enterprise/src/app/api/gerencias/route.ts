...
export async function GET(request: Request) {
  ...
  try {
    for (const base of urls) {
      try {
        const response = await fetch(`${base}/gerencias${suffix}`, {
          method: "GET",
    ...
    console.error("Gerencias GET proxy error:", error);
    return NextResponse.json(
      {
        detail:
          "Error en el proxy de gerencias. Verifique backend local (127.0.0.1:8000) o NEXT_PUBLIC_API_URL.",
      },
      { status: 500 },
    );
  }
