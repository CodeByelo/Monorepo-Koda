...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("Documentos GET proxy error:", error);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const hdrs = await backendHeaders(request);

    // Preserve original Content-Type for multipart uploads (includes boundary)
    if (ct) {
      (hdrs as Record<string, string>)["Content-Type"] = ct;
    }
    ...
    console.error("Documentos POST proxy error:", error);
    return NextResponse.json({ detail: "Error en el proxy de documentos" }, { status: 500 });
  }
}
