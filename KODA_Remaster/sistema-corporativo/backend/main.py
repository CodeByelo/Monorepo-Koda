...
async def startup():
    ...
    try:
        ...
        from database.schema_init import initialize_core_schema
        async with async_db.pool.acquire() as conn:
            await initialize_core_schema(conn)

    except Exception as exc:
        logger.warning(f"No se pudo inicializar el esquema base en startup: {exc}")
...
def _is_admin_master_role(role_name: Optional[str]) -> bool:
    if not role_name:
        return False
    role = str(role_name).strip().lower()
    return role in {"desarrollador", "dev", "developer", "administrador", "admin"}
