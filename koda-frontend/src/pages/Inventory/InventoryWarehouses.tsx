...
const InventoryWarehouses = () => {
  ...
  const handleCreateWarehouse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      const detail = err?.response?.data?.detail || err?.message || 'Error al registrar el almacén.';
                ...
                <div className="flex justify-between items-start flex-wrap gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="bg-amber-400 text-slate-900 font-black text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                        Punto de Venta Oficial
                      </span>
                      <span className="text-teal-200 text-xs font-bold uppercase tracking-wider">{localWh.location}</span>
                    </div>
                    <h3 className="text-2xl font-black uppercase tracking-tight flex items-center gap-2">
                      <Store size={24} className="text-amber-300" />
                      🏪 Tu Local — {localWh.name}
                    </h3>
                    <p className="text-teal-100 text-xs font-medium flex items-center gap-1.5 pt-1">
                      <MapPin size={12} className="text-teal-300" />
                      {localWh.address || 'Dirección no especificada'} • Responsable: {localWh.manager}
                    </p>
                  </div>
                  <div className="flex gap-4 items-center">
                    <div className="bg-white/10 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 text-center">
                      <span className="text-[10px] font-bold uppercase text-teal-200 block">Productos en Local</span>
                      <strong className="text-2xl font-black text-white font-mono">{localWh.products}</strong>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 text-center">
              ...
              <div className="flex justify-between items-center flex-wrap gap-4">
                <div>
                  <h3 className="text-xl font-black uppercase tracking-tight text-slate-800 flex items-center gap-2">
                    <Warehouse size={20} className="text-slate-500" />
                    Tus Almacenes de Depósito
                  </h3>
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider pt-0.5">
                    Depósitos de mercancía e inventario en resguardo
                  </p>
                </div>
                <div className="flex gap-3">
                    ...
                    {depotWarehouses.length === 0 ? (
                      ...
                      <tr key={i} className="group hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-5 px-4 text-xs font-bold text-slate-600 uppercase">
                          <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-[9px] font-black tracking-wider">
                            🏭 ALMACÉN
                          </span>
                        </td>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md p-8 space-y-6 animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Nuevo Almacén / Local</h3>
              <button 
            ...
            <form onSubmit={handleCreateWarehouse} className="space-y-4">
              <div className="space-y-2">
                ...
                <select
                  value={newTipo}
                  onChange={(e) => setNewTipo(e.target.value as 'LOCAL' | 'ALMACEN')}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Ej. Sede Principal Las Mercedes"
