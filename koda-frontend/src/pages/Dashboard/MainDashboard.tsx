...
import { OnboardingChecklist, QuickHelpModal } from '@/components/common/OnboardingGuide';
...
const MainDashboard = () => {
  ...
  const [showHelpModal, setShowHelpModal] = useState(() => searchParams.get('guia') === '1');
  ...
  useEffect(() => {
    if (searchParams.get('guia') === '1') {
      ...
      // Limpiar el searchParam de la url limpiamente
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <header className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-emerald-50 text-[#0b5156] border border-emerald-200 px-4 py-2.5 rounded-xl font-black text-xs uppercase hover:bg-emerald-100 transition-colors active:scale-95"
          >
            <BookOpen size={16} />
            Guía de Inicio
          </button>
      ...
      {/* GUÍA INTERACTIVA DE INICIO RÁPIDO (ONBOARDING) */}
      <section>
        <OnboardingChecklist onOpenHelp={() => setShowHelpModal(true)} />
      </section>

      {/* MODAL GUÍA DE PRINCIPIANTE */}
      <QuickHelpModal isOpen={showHelpModal} onClose={() => setShowHelpModal(false)} />
