...
# ──────────────────────────────────────────────────────────────────────────────
# Redis es OPCIONAL: se usa para token blacklisting (revocación inmediata de
# sesiones), pero el sistema funciona sin él — simplemente no se pueden revocar
# tokens antes de su expiración natural.
#
# Si REDIS_URL no está configurada (vacía o la cadena por defecto
# "redis://localhost:6379/0" en un entorno sin Redis local), se desactiva
# silenciosamente. Si está configurada pero no responde, se advierte UNA sola
# vez para no saturar los logs de Render con miles de líneas idénticas.
# ──────────────────────────────────────────────────────────────────────────────
_REDIS_URL_RAW = os.getenv("REDIS_URL", "").strip()
_redis_warned = False  # Flag para advertir una sola vez

redis_client: Optional[redis.Redis] = None
if _REDIS_URL_RAW:
    try:
        redis_client = redis.from_url(_REDIS_URL_RAW, decode_responses=True)
        # Test de conectividad inmediato (fail-fast en startup, no en cada request)
        redis_client.ping()
        logger.info("✅ Redis conectado correctamente: %s", _REDIS_URL_RAW.split("@")[-1] if "@" in _REDIS_URL_RAW else "(local)")
    except Exception as e:
        logger.warning("⚠️ Redis no disponible (%s). Token blacklisting desactivado — los tokens sólo expiran por TTL.", str(e))
        redis_client = None
        _redis_warned = True
else:
    logger.info("ℹ️ REDIS_URL no configurada. Token blacklisting desactivado — los tokens sólo expiran por TTL.")
...
def blacklist_token(jti: str, expires_in_seconds: int):
    ...
    global _redis_warned
    if not jti or expires_in_seconds <= 0 or redis_client is None:
        return
    try:
...
def get_current_user_from_token(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> Profile:
    ...
    try:
        ...
        #    Solo si Redis está disponible — sin Redis, los tokens solo expiran por TTL.
